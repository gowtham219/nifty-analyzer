...
def fetch_daily_yf(ticker: str, days: int = 200) -> pd.DataFrame:
    """
    More reliable on Streamlit Cloud than yf.download() for daily.
    """
    if yf is None:
        return pd.DataFrame()
    try:
...
# =========================
# Last Trading Day table (requested)
# =========================
def build_last_trading_day_table(daily: pd.DataFrame) -> pd.DataFrame:
    ...
    Adds:
      - %Chg vs PrevClose (for last day)
      - Gap% vs PrevClose (for last day)
    """
    ...
    prev_date = d.index[-2].date()

    def make_row(bar, label, date_):
        ...
        return {
            ...
            "Low": round(l, 2),
            "Close": round(c, 2),
            "%Chg vs PrevClose": None,
            "Gap% vs PrevClose": None,
            "Range(H-L)": round(rng, 2),
            "Body%": (round(body_pct, 0) if not math.isnan(body_pct) else None),
            "ClosePos%": (round(close_pos, 0) if not math.isnan(close_pos) else None),
            "Candle": candle
        }

    table = pd.DataFrame([
        make_row(last_day, "Last Trading Day", last_date),
        make_row(prev_day, "Previous Trading Day", prev_date),
    ])

    try:
        pct = ((float(last_day["Close"]) / float(prev_day["Close"])) - 1) * 100.0
        gap = ((float(last_day["Open"]) / float(prev_day["Close"])) - 1) * 100.0
        table.loc[0, "%Chg vs PrevClose"] = round(pct, 2)
        table.loc[0, "Gap% vs PrevClose"] = round(gap, 2)
    except Exception:
        pass
...
st.caption("Fixed: yfinance MultiIndex handled + manual SCAN + last trading day table + market closed NULL options + debug.")
...
with st.expander("🧪 Debug / Data Health (open if something is blank)", expanded=False):
    ...
    daily_dbg = fetch_daily_yf(yf_ticker, days=200)
...
# ---------------- Last Trading Day Snapshot (YOUR REQUEST) ----------------
with tabs[1]:
